...
#include <exprob_assignment2/Oracle.h>
...
std::vector<exprob_assignment2::ErlOracle> oracle_msgs;
...
bool oracleService(exprob_assignment2::Oracle::Request &req, exprob_assignment2::Oracle::Response &res)
	{
		res.ID = winID;
		return true;
	}
   ...
   for(int i=0; i< msg->name.size(); i++){
	   if (msg->name[i].find("cluedo_link")!= std::string::npos){
		   for(int j=0; j<4;j++){
				...
				exprob_assignment2::ErlOracle oracle_msg;
...
oracle_pub = nh.advertise<exprob_assignment2::ErlOracle>( "/oracle_hint", 0 );
